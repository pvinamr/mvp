...
def build_team_ratings(
    ...
    *,
    ...
    wp_max: float,
    n0: int,
) -> Optional[Tuple[pd.DataFrame, pd.DataFrame, float, float]]:
    ...
    off_rank["epa_per_play_sh"] = (
        (off_rank["epa_per_play"] * off_rank["plays"] + mu_off * n0) / (off_rank["plays"] + n0)
    )
    ...
    mu_off_eckel = (
        float(off_rank["off_eckel_rate"].mean(skipna=True))
        if off_rank["off_eckel_rate"].notna().any()
        else 0.35
    )
    mu_def_eckel = (
        float(def_rank["def_eckel_allowed"].mean(skipna=True))
        if def_rank["def_eckel_allowed"].notna().any()
        else 0.35
    )
...
def matchup_frame(
    ...
    home_off = off_rank[["team", "epa_per_play_sh", "off_plays_pg_raw", "off_eckel_rate"]].rename(
        columns={
            "team": "home_team",
            "epa_per_play_sh": "home_off_epa",
            "off_plays_pg_raw": "home_off_plays_pg",
            "off_eckel_rate": "home_off_eckel",
        }
    )
    away_off = off_rank[["team", "epa_per_play_sh", "off_plays_pg_raw", "off_eckel_rate"]].rename(
        columns={
            "team": "away_team",
            "epa_per_play_sh": "away_off_epa",
            "off_plays_pg_raw": "away_off_plays_pg",
            "off_eckel_rate": "away_off_eckel",
        }
    )
    home_def = def_rank[["team", "epa_allowed_sh", "def_plays_pg_raw", "def_eckel_allowed"]].rename(
        columns={
            "team": "home_team",
            "epa_allowed_sh": "home_def_epa_allowed",
            "def_plays_pg_raw": "home_def_plays_pg",
            "def_eckel_allowed": "home_def_eckel_allowed",
        }
    )
    away_def = def_rank[["team", "epa_allowed_sh", "def_plays_pg_raw", "def_eckel_allowed"]].rename(
        columns={
            "team": "away_team",
            "epa_allowed_sh": "away_def_epa_allowed",
            "def_plays_pg_raw": "away_def_plays_pg",
            "def_eckel_allowed": "away_def_eckel_allowed",
        }
    )

    m = (
        sched_wk.merge(home_off, on="home_team", how="left")
    ...
    plays_pg_all = pd.concat(
        [off_rank["off_plays_pg_raw"], def_rank["def_plays_pg_raw"]], ignore_index=True
    ).dropna()
    if len(plays_pg_all) >= 20:
        ...
        K_LO, K_HI = 55.0, 72.0
    m["K_pair"] = (
        (m["home_off_plays_pg"].fillna(60) + m["away_off_plays_pg"].fillna(60)) / 2.0
    ).clip(K_LO, K_HI)

    # net EPA (sign-correct), with soft clip scale learned from matchups this week
    tmp = (
        sched_wk.merge(home_off[["home_team", "home_off_epa"]], on="home_team", how="left")
    ...
    net_unclipped = (tmp["home_off_epa"] + tmp["away_def_epa_allowed"]) - (
        tmp["away_off_epa"] + tmp["home_def_epa_allowed"]
    )
